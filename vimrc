...
"Immeadiately seach for the word under the cursor in a new tab"
...
if &t_Co > 2
    ...
    if &term =~ "256"
        ...
        if filereadable(expand("$HOME/.vim/bundle/vim-lucius/colors/lucius.vim"))
            colorscheme lucius
            LuciusLightHighContrast
        elseif filereadable(expand("$HOME/.vim/bundle/vim-tomorrow-theme/colors/Tomorrow.vim"))
            colorscheme Tomorrow
        elseif filereadable(expand("$VIMRUNTIME/colors/morning.vim"))
            colorscheme morning
        elseif filereadable(expand("$HOME/.vim/colors/smyck.vim"))
            colorscheme smyck
        else
            colorscheme default
        endif
...
if has('autocmd')
    ...
    if !exists("autocommands_loaded")
        ...
        autocmd BufWritePre *.c,*.cpp,*.h,*.hpp,*.i :call StripTrailingWhitespace()
        ...
        augroup vimrc
            ...
            "			au BufRead *.vim,vimrc,*.vimrc set filetype=vim ts=4 sw=4 tw=500 foldmethod=marker foldcolumn=6
            au BufRead *.vim,vimrc,*.vimrc set filetype=vim ts=4 sw=4 tw=500
        augroup END
...
nnoremap <buffer> <f7> :call <sid>Check()<cr>
...
function CppCheck()
    call FunForQuickfix("cppcheck *.c *.cpp -j 8 --enable=all", "\[%f:%l\]:\ %m")
endfunction
